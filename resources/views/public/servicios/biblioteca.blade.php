  ...
  <section class="relative min-h-[30vh] md:min-h-[35vh] flex items-center overflow-hidden">
    ...
    <div class="relative z-20 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
      <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
        ...
        {{-- Icono decorativo --}}
        <div
          class="hidden md:flex items-center justify-center h-20 w-20 rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 text-4xl animate-float">
          📚
        </div>
      </div>
    </div>
  ...
  <section class="bg-[#faf5ff] py-16 md:py-20">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      @php
        ...
        // Ensure 'Otros' is at the bottom by sorting the keys
        $groupedItems = $groupedItems->sortBy(function($items, $key) {
            return $key === 'Otros' ? 'ZZZZ' : $key;
        });
      @endphp

      @if($groupedItems->isNotEmpty())
        @foreach($groupedItems as $carrera => $items)
          {{-- Sección para cada Carrera o "Otros" --}}
          <div class="mb-8 {{ $loop->first ? '' : 'mt-16' }} flex items-center gap-3 animate-fade-in-up">
            <div class="h-8 w-1 bg-[#6b3f8c] rounded-full"></div>
            <h3 class="text-xl md:text-2xl font-bold text-[#4a2e6e]">{{ $carrera === 'Otros' ? 'Documentos Generales (Otros)' : 'Programa: ' . $carrera }}</h3>
          </div>
          ...
          <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            @foreach($items as $index => $item)
              <div
                ...
                <div
                  class="relative h-48 bg-gradient-to-br from-[#faf5ff] to-[#6b3f8c]/10 border-b-2 border-[#c9a227]/20 overflow-hidden">
                  {{-- Patrón de fondo --}}
      ...
      @else
        ...
        <div class="bg-white rounded-3xl shadow-xl border-2 border-dashed border-[#c9a227]/30 p-16 text-center">
          <div class="max-w-md mx-auto">
            ...
            <h3 class="text-2xl font-bold text-[#4a2e6e] mb-3">Repositorio en construcción</h3>
            <p class="text-gray-600 mb-8">
              Aún no hay archivos disponibles en el repositorio digital. Estamos trabajando para agregar nuevos recursos.
            </p>
            ...
            <p class="text-sm text-gray-500 mt-8 italic">
              Administrador: Sube archivos desde el panel de administración (Repositorio).
            </p>
          </div>
        </div>
      @endif
